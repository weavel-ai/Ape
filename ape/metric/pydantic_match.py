...
class PydanticMatchMetric(BaseMetric):
    ...
    async def compute(self, gold: DataItem, pred: Any, trace: Optional[Dict] = None) -> float:
        ...
        async def compare_values(gold_value, pred_value):
            ...
            if isinstance(gold_value, dict) and isinstance(pred_value, dict):
                ...
                res: dict = await binary_judge(ground_truth=gold_value, prediction=pred_value)
        ...
        async def compare_lists(list1, list2):
            ...
            if not self.consider_list_order:
                ...
                dicts1 = set(json.dumps(item, sort_keys=True) for item in list1 if isinstance(item, dict))
                dicts2 = set(json.dumps(item, sort_keys=True) for item in list2 if isinstance(item, dict))
                ...
                return sum(await compare_values(g1, g2) for g1, g2 in zip(list1, list2)) / len(list1) if len(list1) == len(list2) else 0.0
