...
class TextGradientTrainerV3(BaseTrainer):
    def __init__(
        self,
    ...
    async def fit(
        ...
        # Combine trainset and valset for evaluation
        ...
        # Evaluate initial prompt on combined eval_set
        _, _, best_eval_score = await self._evaluate_dataset(eval_set, best_prompt)
        ...
        for batch_start in tqdm(range(0, len(shuffled_trainset), self.batch_size), desc="Training Batches"):
            ...
            while retry_count < self.max_proposals_per_step and not success:
                ...
                if self.paraphraser is not None:
                    ...
                    if new_batch_score.score > best_batch_score.score:
                        # Evaluate new_prompt on eval_set instead of buffer_trainset
                        _, _, new_eval_score = await self._evaluate_dataset(eval_set, new_prompt)

                        prompt_history_queue.append(
                            {
                                "prompt": new_prompt,
                                "score": new_eval_score.score
                            }
                        )

                        # Compare new score with the current best score
                        if new_eval_score.score > best_eval_score.score:
                            print(f"Trial {retry_count + 1}: Score Improved: {best_eval_score.score} -> {new_eval_score.score}")
                            best_prompt = new_prompt
                            best_eval_score = new_eval_score
                            ...
                            print(f"Trial {retry_count + 1}: Score Not Improved: {best_eval_score.score} -> {new_eval_score.score}")
            ...
            report.scores.append({
                ...
                "score": best_eval_score.score
            })
            if best_eval_score.score == 1.0:
                return best_prompt, report
